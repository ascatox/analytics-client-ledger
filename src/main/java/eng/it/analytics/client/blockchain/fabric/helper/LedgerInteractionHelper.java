package eng.it.analytics.client.blockchain.fabric.helper;

import eng.it.analytics.client.blockchain.fabric.config.*;
...
final public class LedgerInteractionHelper {
    ...
    public LedgerInteractionHelper(ConfigManager configManager, Organization organization) throws SmartClientException {
        this(configManager, organization, null);

    }

    public LedgerInteractionHelper(ConfigManager configManager, Organization organization, String userName) throws SmartClientException
    ...
    private void setup() throws Exception {
        ...
        if (null == channel || !channel.isInitialized() || channel.isShutdown()) {
            ...
            throw new SmartClientException("Channel is not initialized");
        }
    ...
    public void controlInstalledChaincodeOnPeers(Chaincode chaincode) throws SmartClientException {
        ...
        for (Peer peer : channel.getPeers()) {
            try {
                if (!checkInstalledChaincode(peer, chaincode)) {
                    throw new SmartClientException(format("Peer %s is missing chaincode whith name: %s, path: %s, version: %s",
    ...
    private boolean checkInstalledChaincode(Peer peer, Chaincode chaincode) throws SmartClientException
    {
    ...
    public void controlInstantiatedChaincodeOnPeers(Chaincode chaincode) throws SmartClientException {
        log.debug("Checking installed chaincode on all peer: %s, at version: %s, on peer: %s", chaincode.getName(), chaincode.getVersion(), channel.getPeers());
        for (Peer peer : channel.getPeers()) {
            if (!checkInstantiatedChaincode(peer, chaincode)) {
                try {
                    throw new SmartClientException(format("Peer %s has not installed chaincode with name: %s, path: %s, version: %s",
                            peer.getName(), chaincode.getName(), chaincode.getPath(), chaincode.getVersion()));
                } catch (SmartClientException e) {
                    log.error(e);
                    throw new SmartClientException(format("Peer %s has not installed chaincode with name: %s, path: %s, version: %s",
    ...
    private boolean checkInstantiatedChaincode(Peer peer, Chaincode chaincode) throws SmartClientException {
    ...
    public InvokeReturn invokeChaincode(String functionName, List<String> args) throws SmartClientException {
        try {
    ...
    public List<QueryReturn> queryChainCode(String functionName, List<String> args, BlockEvent.TransactionEvent transactionEvent) throws SmartClientException {
        try {
            ...
            log.error(e);
            throw new SmartClientException("Failed during chaincode query with error : " + e.getMessage());
        }
    }
